...
class PlatformDataProvider {
  ...
  protected function normalizePlatformNode(NodeInterface $platform): array {
    return [
      ...
      'cpu_socket_count' => $this->getCpuSocketCount($platform),
  ...
  protected function getCpuSocketCount(NodeInterface $platform): ?int {
    foreach ([
      'field_cpu_socket_count',
      'field_cpu_sockets_count',
      'field_socket_count',
      'field_cpu_socket',
      'field_cpu_sockets',
    ] as $field) {
      $value = $this->getNumeric($platform, $field);
      if ($value !== NULL) {
        return $value;
      }
    }
