...
namespace Drupal\server_configurator\Infrastructure\Repository;
...
final class ServerRepository {
  ...
  public function buildTableOptions(array $servers): array {
    ...
    foreach ($servers as $server) {
      ...
      $lanPortsSpeed = $platform instanceof NodeInterface ? $this->getPlatformTextValue($platform, 'field_lan_ports_speed') : '';
      $storage_form_factor =  $platform instanceof NodeInterface ? $this->getPlatformTextValue($platform, 'field_storage_form_factor') : '';

      $options[(int) $server->id()] = [
        'title' => $server->label(),
        'platform' => $platform instanceof NodeInterface ? $platform->label() : '—',
        'lan' => $lanPortsSpeed !== '' ? $lanPortsSpeed : '—',
        'max_drives' => $maxDrives,
        'storage_form_factor' => $storage_form_factor,
      ];
    }
  ...
  public function buildServerPlatformMap(array $servers): array {
    ...
    foreach ($servers as $server) {
      ...
      $lanPortsSpeed = $platform instanceof NodeInterface ? $this->getPlatformTextValue($platform, 'field_lan_ports_speed') : '';
      $storageFormFactor = $platform instanceof NodeInterface ? $this->getPlatformTextValue($platform, 'field_storage_form_factor') : '';
  ...
  protected function getPlatformTextValue(NodeInterface $platform, string $fieldName): string {
