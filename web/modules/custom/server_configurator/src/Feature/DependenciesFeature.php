...
class DependenciesFeature {
  ...
  public function alter(array &$form, FormStateInterface $form_state, ConfiguratorDefinition $definition): void {
    ...
    if ($cpu_vendor_element === NULL || $cpu_generation_element === NULL || $platform_element === NULL || $form_factor_element === NULL) {
      return;
    }

    $this->attachAjax($cpu_vendor_element, $cpu_generation_element, $platform_element, $form_factor_element);
    $this->apply($form_state, $cpu_generation_element, $platform_element, $definition);
  }

  public function attachAjax(array &$cpu_vendor_element, array &$cpu_generation_element, array &$platform_element, array &$form_factor_element): void {
  ...
  public function apply(FormStateInterface $form_state, array &$cpu_generation_element, array &$platform_element, ConfiguratorDefinition $definition): void {
    ...
    $this->applyPlatformDependency($platform_element, $cpu_generation_value, $form_factor_value);
  }
  ...
  protected function applyPlatformDependency(array &$platform_element, $cpu_generation_value, $form_factor_value): void {
    if ($this->locator->hasValue($cpu_generation_value) && $this->locator->hasValue($form_factor_value)) {
      $this->locator->setViewArguments($platform_element, [$cpu_generation_value, $form_factor_value], '- Выберите платформу -');
    }
