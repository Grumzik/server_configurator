  ...
  Drupal.serverConfiguratorRestore = {
    ...
    restoreStorage(state) {
      ...
      rows.forEach((row) => {
        ...
        const type = row.querySelector('.storage_types:checked')?.value;
        const count = row.querySelector('.storage_count')?.value;
        const volume = row.querySelector('.storage-volume input:checked')?.value;
