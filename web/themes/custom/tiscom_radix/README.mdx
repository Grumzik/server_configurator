...
tiscom_radix theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of tiscom_radix theme and run the following commands:
`nvm use`
...
`drush en tiscom_radix -y`
`drush config-set system.theme default tiscom_radix -y`
